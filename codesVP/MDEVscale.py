...
boxsize=15
runindex = 0
densityarr = np.linspace(1,2,10)
if runindex < 0 or runindex >= len(densityarr):
    raise ValueError(f"runindex {runindex} is outside densityarr length {len(densityarr)}")
simbox = SimCell(xysize=[boxsize, boxsize])
amm = Specie("NH3", name="NH3")
densitygcm3 = densityarr[runindex] #gcm3
...
def simpleMD(init_conf, temp, calc, fname, s, T): 
    ...
    np.savetxt(
        ...
        data,
        header=(
            "time_fs temperature_K energy_eV_per_atom kinetic_energy_eV_per_atom "
...
simpleMD(
    ...
    temp=150,
    calc=mace_calc,
    fname=os.path.join(MD_RESULTS_DIR, f"mace_150K_density_{densitygcm3}.xyz"),
    s=200,
    T=1000,
)
