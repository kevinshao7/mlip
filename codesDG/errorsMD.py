...
LEGACY_MD_RESULTS_DIR = PROJECT_ROOT / "codes" / "MDresults"
THERMO_COLUMNS = (
    "time_fs",
    "temperature_K",
    "pressure_GPa",
...
XYZ_OBSERVABLES = (
    ...
    "xyz_total_momentum_x",
    "xyz_total_momentum_y",
    "xyz_total_momentum_z",
    "xyz_total_momentum_norm",
)
...
OBSERVABLE_LABELS = {
    ...
    "kinetic_energy_eV_per_atom": "kinetic energy per atom (eV/atom)",
    "total_energy_eV_per_atom": "total energy per atom (eV/atom)",
    ...
    "xyz_total_momentum_x": "total momentum x (ASE units)",
    "xyz_total_momentum_y": "total momentum y (ASE units)",
    "xyz_total_momentum_z": "total momentum z (ASE units)",
    "xyz_total_momentum_norm": "total momentum norm (ASE units)",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--plots-dir",
        default=str(MD_RESULTS_DIR / "plots"),
        help="Directory for generated plots.",
    )
    parser.add_argument(
        "--summary-json",
        default=str(MD_RESULTS_DIR / "errorsMD_summary.json"),
...
def discover_default_inputs(args: argparse.Namespace) -> None:
    search_dirs = [MD_RESULTS_DIR]
    if LEGACY_MD_RESULTS_DIR.exists():
        search_dirs.append(LEGACY_MD_RESULTS_DIR)

    if args.thermo is None:
        args.thermo = [
            str(path)
            for directory in search_dirs
            for path in sorted(directory.glob("*_thermo.txt"))
        ]
    if args.xyz is None:
        args.xyz = [
            str(path)
...
def parse_extended_xyz_headers(path: Path) -> dict[str, np.ndarray]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        while True:
            ...
            momentum_slice = property_slice(fields.get("Properties", ""), "momenta")
            ...
            total_momentum = np.zeros(3, dtype=float)
            has_momenta = momentum_slice is not None
            for _ in range(natoms):
                atom_fields = handle.readline().split()
                if has_momenta:
                    try:
                        total_momentum += np.asarray(
                            atom_fields[momentum_slice], dtype=float
                        )
                    except (IndexError, ValueError):
                        has_momenta = False

            if has_momenta:
                rows["xyz_total_momentum_x"].append(float(total_momentum[0]))
                rows["xyz_total_momentum_y"].append(float(total_momentum[1]))
                rows["xyz_total_momentum_z"].append(float(total_momentum[2]))
                rows["xyz_total_momentum_norm"].append(float(np.linalg.norm(total_momentum)))
            else:
                rows["xyz_total_momentum_x"].append(float("nan"))
                rows["xyz_total_momentum_y"].append(float("nan"))
                rows["xyz_total_momentum_z"].append(float("nan"))
                rows["xyz_total_momentum_norm"].append(float("nan"))

    return {name: np.asarray(values, dtype=float) for name, values in rows.items()}


def property_slice(properties: str, name: str) -> slice | None:
    parts = properties.split(":")
    column = 0
    for i in range(0, len(parts) - 2, 3):
        prop_name = parts[i]
        try:
            n_cols = int(parts[i + 2])
        except ValueError:
            return None
        if prop_name == name:
            return slice(column, column + n_cols)
        column += n_cols
    return None
...
def load_thermo(path: Path) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    ...
    potential_energy_eV_per_atom = data[:, 3]
    if data.shape[1] >= 6:
        kinetic_energy_eV_per_atom = data[:, 4]
        total_energy_eV_per_atom = data[:, 5]
    else:
        kinetic_energy_eV_per_atom = 1.5 * KB_EV_PER_K * temperature_K
        total_energy_eV_per_atom = (
    ...
    series = {
        ...
        "pressure_GPa": data[:, 2],
        "energy_eV_per_atom": potential_energy_eV_per_atom,
        "kinetic_energy_eV_per_atom": kinetic_energy_eV_per_atom,
...
def make_total_energy_temperature_plot(
    plots_dir: Path,
    plot_title: str,
    display_name: str,
    time_fs: np.ndarray,
    total_energy_eV_per_atom: np.ndarray,
    temperature_K: np.ndarray,
    drop_fraction: float,
) -> None:
    time_fs = np.asarray(time_fs, dtype=float)
    total_energy_eV_per_atom = np.asarray(total_energy_eV_per_atom, dtype=float)
    temperature_K = np.asarray(temperature_K, dtype=float)
    n = min(len(time_fs), len(total_energy_eV_per_atom), len(temperature_K))
    if n < 2:
        return
    time_fs = time_fs[:n]
    total_energy_eV_per_atom = total_energy_eV_per_atom[:n]
    temperature_K = temperature_K[:n]
    finite = (
        np.isfinite(time_fs)
        & np.isfinite(total_energy_eV_per_atom)
        & np.isfinite(temperature_K)
    )
    time_fs = time_fs[finite]
    total_energy_eV_per_atom = total_energy_eV_per_atom[finite]
    temperature_K = temperature_K[finite]
    drop = int(len(time_fs) * drop_fraction)
    time_fs = time_fs[drop:]
    total_energy_eV_per_atom = total_energy_eV_per_atom[drop:]
    temperature_K = temperature_K[drop:]
    if len(time_fs) < 2:
        return
    ...
    energy_line = ax_energy.plot(
        ...
        color="tab:blue",
        lw=1.4,
        label="total energy per atom",
    )
    ax_energy.set_xlabel("time (fs)")
    ax_energy.set_ylabel("total energy per atom (eV/atom)", color="tab:blue")
    ...
    lines = energy_line + temp_line
    ax_energy.legend(lines, [line.get_label() for line in lines], loc="best", fontsize=8)
    ax_energy.set_title(f"NVE diagnostic: {display_name}")
    ax_energy.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(
        plots_dir / f"errorsMDplot_{plot_title}_total_energy_temperature.png",
        dpi=200,
    )
    plt.close(fig)


def first_xyz_natoms(path: Path) -> int | None:
    try:
        with path.open("r", encoding="utf-8") as handle:
...
def make_total_energy_conservation_plot(
    plots_dir: Path,
    plot_title: str,
    display_name: str,
    time_fs: np.ndarray,
    ...
    time_fs = np.asarray(time_fs, dtype=float)
    total_energy_eV_per_atom = np.asarray(total_energy_eV_per_atom, dtype=float)
    n = min(len(time_fs), len(total_energy_eV_per_atom))
    if n < 2:
        return
    time_fs = time_fs[:n]
    energy = total_energy_eV_per_atom[:n]
    ylabel = "total energy per atom (eV/atom)"
    label = "total energy per atom"
    if natoms is not None:
        energy = energy * natoms
        ylabel = "total system energy (eV)"
        label = "total system energy"
    finite = np.isfinite(time_fs) & np.isfinite(energy)
    time_fs = time_fs[finite]
    ...
    ax_energy.plot(time_fs, energy, lw=1.4, label=label)
    ax_energy.set_ylabel(ylabel)
    ax_energy.set_title(f"Total energy conservation: {display_name}")
    ax_energy.legend(loc="best", fontsize=8)
    ax_energy.grid(True, alpha=0.3)

    ax_drift.plot(time_fs, drift, color="tab:red", lw=1.2, label="drift from first frame")
    ax_drift.axhline(0.0, color="k", lw=0.8, alpha=0.5)
    ax_drift.set_xlabel("time (fs)")
    ax_drift.set_ylabel("energy drift (eV)" if natoms is not None else "energy drift (eV/atom)")
    ax_drift.legend(loc="best", fontsize=8)
    ax_drift.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(
        plots_dir / f"errorsMDplot_{plot_title}_total_energy_conservation.png",
        dpi=200,
    )
    plt.close(fig)


def make_total_momentum_conservation_plot(
    plots_dir: Path,
    plot_title: str,
    display_name: str,
    time_fs: np.ndarray,
    ...
    arrays = [
        np.asarray(time_fs, dtype=float),
        np.asarray(px, dtype=float),
        np.asarray(py, dtype=float),
        np.asarray(pz, dtype=float),
        np.asarray(pnorm, dtype=float),
    ]
    n = min(len(array) for array in arrays)
    if n < 2:
        return
    time_fs, px, py, pz, pnorm = [array[:n] for array in arrays]
    finite = (
        np.isfinite(time_fs)
        & np.isfinite(px)
        & np.isfinite(py)
    ...
    plots_dir.mkdir(parents=True, exist_ok=True)
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.plot(time_fs, px, lw=1.1, label="Px")
    ax.plot(time_fs, py, lw=1.1, label="Py")
    ax.plot(time_fs, pz, lw=1.1, label="Pz")
    ax.plot(time_fs, pnorm, color="k", lw=1.4, label="|P|")
    ax.axhline(0.0, color="k", lw=0.8, alpha=0.4)
    ax.set_xlabel("time (fs)")
    ax.set_ylabel("total system momentum (ASE units)")
    ax.set_title(f"Total momentum conservation: {display_name}")
    ax.legend(loc="best", fontsize=8)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(
        plots_dir / f"errorsMDplot_{plot_title}_total_momentum_conservation.png",
        dpi=200,
    )
    plt.close(fig)
...
def analyze_thermo_files(
    ...
    for path_text in paths:
        ...
        xyz_guess = path.with_name(path.name.replace("_thermo.txt", ".xyz"))
        make_total_energy_conservation_plot(
            plots_dir=plots_dir,
            plot_title=safe_label(path.stem),
            display_name=path.stem,
            time_fs=time_fs,
            total_energy_eV_per_atom=series["total_energy_eV_per_atom"],
            natoms=first_xyz_natoms(xyz_guess),
        )
        make_total_energy_temperature_plot(
            plots_dir=plots_dir,
            plot_title=safe_label(path.stem),
            display_name=path.stem,
            time_fs=time_fs,
...
def analyze_xyz_files(
    ...
    for path_text in paths:
        ...
        if thermo_guess.exists():
            ...
            dt_fs = 1.0
        if time_fs is None:
            plot_time_fs = np.arange(len(next(iter(series.values()))), dtype=float) * dt_fs
        else:
            plot_time_fs = time_fs
        make_total_momentum_conservation_plot(
            plots_dir=plots_dir,
            plot_title=safe_label(path.stem),
            display_name=path.stem,
            time_fs=plot_time_fs,
            px=series["xyz_total_momentum_x"],
            py=series["xyz_total_momentum_y"],
            pz=series["xyz_total_momentum_z"],
            pnorm=series["xyz_total_momentum_norm"],
        )
