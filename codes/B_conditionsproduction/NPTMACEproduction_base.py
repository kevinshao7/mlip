...
densitygcm3 = 0.0495 # g/cm3, from selected Uranus profile row
pressuregpa = 0.11 # GPa
targetmolecules = 100
moleculemass = 18.01528 # grams per mol, composition-weighted by generated cases
tempramptime = 10*1000*units.fs
MDtimestep = 0.5*units.fs
totaltimesteps = 50000
saveinterval =5
T_initial=300 #always keep at room temperature
T_final = 759  # Uranus profile target temperature for NPT production.
composition_label = "R0p0"
ammonia_water_ratio = 0.0 # NH3/H2O molar ratio
...
simpleMD(
    ...
    fname=os.path.join(
        MD_RESULTS_DIR,
        f"production_seed_{RUN_SEED}_P_{pressuregpa:g}GPa_T_{T_final:g}K_{composition_label}_density_{densitygcm3}.xyz",
    ),
    checkpoint_fname=os.path.join(
        MD_RESULTS_DIR,
        f"checkpoint_seed_{RUN_SEED}_P_{pressuregpa:g}GPa_T_{T_final:g}K_{composition_label}_density_{densitygcm3}.xyz",
    ),
    s=saveinterval,
    T=totaltimesteps
)
